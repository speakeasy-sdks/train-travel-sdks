...
class Trips:
    ...
    def get_trips(self, request: operations.GetTripsRequest, accept_header_override: Optional[GetTripsAcceptEnum] = None) -> operations.GetTripsResponse:
        r"""Get available train trips
        Returns a list of available train trips between the specified origin and destination stations on the given date, and allows for filtering by bicycle and dog allowances.
        """
        hook_ctx = HookContext(operation_id='get-trips', oauth2_scopes=[], security_source=self.sdk_configuration.security)
        ...
        query_params = { **utils.get_query_params(operations.GetTripsRequest, request), **query_params }
        ...
        res = operations.GetTripsResponse(http_meta=components.HTTPMetadata(request=req, response=http_res), headers=None)
        
        if http_res.status_code == 200:
            res.headers = http_res.headers
            
            if utils.match_content_type(http_res.headers.get('Content-Type'), 'application/json'):                
                out = utils.unmarshal_json(http_res.text, Optional[operations.GetTripsResponseBody])
